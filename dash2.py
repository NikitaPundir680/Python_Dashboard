...
CPI_df=pd_2.groupby(['month','Month','CPI']).agg({'Weekly_Sales':'sum'}).reset_index()
CPI_df['CPI'] = CPI_df['CPI'].apply(lambda x: 'CPI'+" "+str(x))
CPI_df['Weekly_Sales'] = CPI_df['Weekly_Sales'].round(1)
...
unemp_df=pd_2.groupby(['month','Month','Unemployment']).agg({'Weekly_Sales':'sum'}).reset_index()
unemp_df['Unemployment'] = unemp_df['Unemployment'].apply(lambda x: 'Unemployment'+" "+str(x))
unemp_df['Weekly_Sales'] = unemp_df['Weekly_Sales'].round(1)
