...
def update_cards(base,comparison):
    ...
    base_st_ct = df.loc[df['Month']==base,'Store'].drop_duplicates().count()
    comp_st_ct = df.loc[df['Month']==comparison,'Store'].drop_duplicates().count()
    diff_st= np.round(base_st_ct-comp_st_ct,1)

    weekly_base = weekly_sales.loc[weekly_sales['Month'] == base].reset_index()
    weekly_comp = weekly_sales.loc[weekly_sales['Month'] == comparison].reset_index()
    ...
    dept_base = dept_df.loc[store_df['Month'] == base].sort_values('Weekly_Sales',ascending = False).reset_index()[:10]
    dept_base = dept_base.rename(columns = {'Weekly_Sales','Weekly_Sales_base'})
    dept_comp = dept_df.loc[store_df['Month'] == comparison].sort_values('Weekly_Sales',ascending = False).reset_index()[:10]
...
navbar = dbc.Navbar( id = 'navbar', children = [
    dbc.Row([
        dbc.Col(
            dbc.NavbarBrand("Retail Sales Dashboard", style = {'color':'White', 'Textalign':'center','fontSize':'40px','fontFamily':'Times New Roman'}
                            )
